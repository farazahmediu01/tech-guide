...
> **Archived from:** <https://agentfactory.panaversity.org/docs/Building-Agent-Factories/vector-databases-rag-langchain/rag-architecture-patterns-capstone>  
...
GET /tasks?status=pending GET /tasks?status=in_progress
...
A task with dependencies cannot be marked complete until all dependency tasks are completed. The API returns 400 Bad Request if you try.
