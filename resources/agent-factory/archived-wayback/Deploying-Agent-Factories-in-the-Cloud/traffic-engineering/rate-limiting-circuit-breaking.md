...
> **Archived from:** <https://agentfactory.panaversity.org/docs/Deploying-Agent-Factories-in-the-Cloud/traffic-engineering/rate-limiting-circuit-breaking>  
...
BackendTrafficPolicy is an Envoy Gateway extension CRD that configures traffic behavior between Envoy proxies and your backend services. While HTTPRoute controls _where_ traffic goes, BackendTrafficPolicy controls _how_ that traffic behaves.
...
Anonymous users (no `x-user-id` header) should get lower limits than authenticated users. Use the `invert` field to match requests _without_ a header.
