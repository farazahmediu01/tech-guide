...
> **Archived from:** <https://agentfactory.panaversity.org/docs/Deploying-Agent-Factories-in-the-Cloud/helm-charts/values-deep-dive>  
...
AnswerNested—group by concept (agent._, database._ , redis._, monitoring._)

  2. How do you access `agent.config.logLevel` in a template? 

Answer`{{ .Values.agent.config.logLevel }}`

  3. What's wrong with organizing values by template file (deployment._, service._ , configMap.*)? 
...
**Organizing by template file**|  Scatters related config (database.* split across deployment._, service._)| Organize by concept, not by where used in templates  
...
Ask yourself:
  ...
  * Did it organize values by concept (agent._, database._) not by template file?
